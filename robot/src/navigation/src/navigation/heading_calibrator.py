...
class HeadingCalibrator:
    ...
    def update(self, cur_x: float, cur_y: float, raw_heading: float) -> bool:
        ...
        rospy.loginfo_throttle(
            0.5, f"Kalibriere... Gefahren: {distance_driven:.2f}m / {self._p.calib_distance:.1f}m")

        calib_time = (rospy.Time.now() - self.calib_start_time).to_sec()

        if distance_driven >= self._p.calib_distance and calib_time > self._p.calib_min_time:
            return self._finalize(calib_dx, calib_dy, raw_heading)
        return False
    ...
    def _finalize(self, calib_dx, calib_dy, raw_heading) -> bool:
